...
def adaptive_max_pool(input, size):
    #print('Inside adaptibv_max_pool Input shape: {} '.format(input.shape))
...
def roi_pooling_ims(input, rois, size=(7, 7), spatial_scale=1.0):
    ...
    for i in range(num_rois):
        roi = rois[i]
        #print('Inside ROI Pooling layer, input: {}, rois: {}'.format(input.shape,roi))
        # im = input.narrow(0, im_idx, 1)
        im = input.narrow(0, i, 1)[..., roi[1]:(roi[3] + 1), roi[0]:(roi[2] + 1)]
        #if im.shape[2] ==0 or im.shape[3]==0:
            #print('Inside ROI Pooling layer, im has zero: printing im: {}'.format(im))
        output.append(adaptive_max_pool(im, size))
